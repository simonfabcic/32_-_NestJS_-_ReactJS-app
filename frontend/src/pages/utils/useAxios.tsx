...
const useAxios = () => {
  const { authTokens, setUserAccessToken, setAuthTokens, logoutUser } =
  ...
  axiosInstance.interceptors.request.use(async (req) => {
    ...
    if (authTokens) {
      ...
      const isAccessTokenExpired =
        userAccessToken.exp &&
        dayjs.unix(userAccessToken.exp).diff(dayjs(), "second") < 10;
      if (isAccessTokenExpired) {
        const userRefreshToken = jwtDecode(authTokens.refresh);
        const isRefreshTokenExpired =
          userRefreshToken.exp &&
          dayjs.unix(userRefreshToken.exp).diff(dayjs(), "second") < 10;
        if (isRefreshTokenExpired) {
          // console.log("Refreshing token expired!")
          logoutUser();
          return Promise.reject(
            new Error("Both tokens expired. User needs to log in again."),
          );
        } else {
          const response = await axios.post(
            `${baseURL}/core-api-v1/token/refresh/`,
            {
