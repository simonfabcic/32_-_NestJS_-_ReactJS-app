...
const Users = () => {
    ...
    const [paginationPageNumbers, setPaginationPageNumbers] = useState<
        number[]
    >([]);

    const AvatarCell: React.FC<{ avatar_image_src_url: string }> = ({
        avatar_image_src_url,
    }) => {
        return (
            <td className="h-full">
                {avatar_image_src_url === null ? (
                    ...
                    <img
                        src={baseURL + avatar_image_src_url}
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                if (response.data.rows.length < 1)
                    setLoadingState(
                        "Didn't receive any profiles. Nothing to show.",
                    );
                let currentPage =
                    response.data.pagination_description.current_page;
        ...
        try {
            ...
            if (response.status === 204) {
                setTableData((prevTableState) => ({
                    ...
                    pagination_description:
                        prevTableState.pagination_description,
                }));
            }
        ...
        <>
            ...
            {tableData.headers.length > 0 && tableData.rows.length > 0 ? (
                <table className="w-full text-center">
                  ...
                  ${
                      deleteButtonPressed &&
                      deleteButtonPressedOnUserId == row.id
                          ? "bg-red-200"
                          : "even:bg-gray-400"
                  }
                `}
                                    ...
                                    <td className="space-x-2">
                                        ...
                                        <button
                                            ...
                                            onClick={() => {
                                                ...
                                                setDeleteButtonPressedOnUserId(
                                                    row.id,
                                                );
                                            }}
                                            onMouseDown={() => {
                            ...
                            <button
                                onClick={() =>
                                    setPaginationWantedPage(
                                        tableData.pagination_description
                                            .total_pages,
                                    )
                                }
